        ...
        {messages.items.map((message, index) => {
          return (
            message.title != 'Notice' ?
              ...
              :
              <Snackbar open={message.open} key={index} anchorOrigin={{ vertical: "top", horizontal: "center" }} style={{ top: '300px' }}>
                <Alert
                  variant="filled"
                  icon={false}
                  severity={message.severity}
                  onClose={handleClose(message)}
                  // NOTE (appleseed): mui includes overflow-wrap: "break-word", but word-break: "break-word" is needed for webKit browsers
                  style={{ wordBreak: "break-word", width: "700px", height: '100px', 'background-color': 'rgb(0 148 253)' }}
                >
                  <AlertTitle style={{'font-size':'25px'}}>{message.title}</AlertTitle>
                  <p style={{'font-size':'20px'}}>{message.text}</p>
                  {/* <Linear message={message} /> */}
                </Alert>
              </Snackbar>
          );
