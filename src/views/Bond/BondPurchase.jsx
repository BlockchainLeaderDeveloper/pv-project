...
import { bondAsset, calcBondDetails, changeApproval,changeApproval_HOMGOLD } from "../../slices/BondSlice";
...
function BondPurchase({ bond, slippage, recipientAddress }) {
  ...
  const hasAllowance_GOLD = useCallback(() => {
    return bond.allowance_HOMGOLD > 0;
  }, [bond.allowance_HOMGOLD]);
  ...
  const onSeekApproval_HOMGOLD = async () => {
    dispatch(changeApproval_HOMGOLD({ address, bond, provider, networkID: chainID }));
  };
      ...
      <Box display="flex" justifyContent="space-around" flexWrap="wrap">
        {isAllowanceDataLoading ? (
          ...
          <>
            {!hasAllowance() ? (
              ...
              !hasAllowance_GOLD() ? (
                <div className="help-text">
                  <em>
                    <Typography variant="body1" align="center" color="textSecondary">
                      First time using <b>HOM-GOLD</b>? <br /> Please approve HOM DAO to use your{" "}
                      <b>HOM-GOLD</b> for bonding.
                    </Typography>
                  </em>
                </div>
              ) : (
            ...
            {hasAllowance() && hasAllowance_GOLD() ? (
              <Button
                variant="contained"
                color="primary"
                id="bond-btn"
              ...
              !hasAllowance() && !hasAllowance_GOLD() ? (
                ...
                <Button
                  ...
                  id="bond-approve-btn"
                  className="transaction-button"
                  disabled={isPendingTxn(pendingTransactions, "approve_HOMGOLD")}
                  onClick={onSeekApproval_HOMGOLD}
                >
                  {txnButtonText(pendingTransactions, "approve_HOMGOLD", "Approve")}
                </Button>
              )
            )}
          </>
