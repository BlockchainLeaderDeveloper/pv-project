...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: IBaseAsyncThunk, { dispatch }) => {
    ...
    const sXPVMainContract = new ethers.Contract(addresses[networkID].SXPV_ADDRESS as string, sBHD, provider);
    const XPVContract = new ethers.Contract(addresses[networkID].XPV_ADDRESS as string, ierc20Abi, provider);
    const XPVBalance = await XPVContract.balanceOf(addresses[networkID].STAKING_ADDRESS);
...
//for presale
    const stakingTVL = (XPVBalance * (marketPrice/1000000) ) / 1000000000 ;  //(XPVBalance * (PresalePrice/1000000) ) / 1000000000
    const circ = await sXPVMainContract.circulatingSupply();
    const circSupply = circ / 1000000000;
    const total = await XPVContract.totalSupply();
...
const loadMarketPrice = createAsyncThunk("app/loadMarketPrice", async ({ networkID, provider }: IBaseAsyncThunk) => {
  ...
  try {
    marketPrice = await getMarketPrice({ networkID, provider });
     marketPrice = marketPrice / Math.pow(10, 18);
  } catch (e) {
    marketPrice = 80;
  }
  return { marketPrice };
