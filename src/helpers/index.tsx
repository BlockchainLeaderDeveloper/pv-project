...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const dai_address = dai.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(dai_address, PairContract, provider);
  const reserves = await pairContract.getReserves();
  const marketPrice = (reserves[1] * 1000000000 )/ reserves[0];
...
export function getTokenImage(name: string) {
  if (name === "XPV") return getBhdTokenImage();
  if (name === "sXPV") return getSbhdTokenImage();
}
